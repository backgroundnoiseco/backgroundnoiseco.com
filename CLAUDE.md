...
**Verified out to 2560px wide** (and 2560×1440): the hero composition, the rail, and the project phone/info pair all hold a *constant* offset from the 644px content band at every width past the 644px cap — nothing drifts against the background, and there's no horizontal overflow. Past-cap widths just grow the gutters.
...
- The badge is **anchored to the third concentric rectangle** — `top: calc(var(--ring-step) * 2)`, `right: calc(var(--cap) + var(--ring-step) * 2)` — so its top and right edges land exactly on ring 3's, verified at 0.0px across 360→1920. `--ring-step` (40px) **must stay equal to `STEP` in `js/site.js`**, which is what actually spaces the rings; move one and the badge stops sitting on a ring. Retarget it by changing the multiplier — `* 1` is ring 2, `* 3` is ring 4. That is only expressible because its containing block is `.panel--hero` rather than the `<h1>`; the h1 is vertically centred and moves with the window. Only `width` is a keyframe pair now, so resizing the badge no longer drags its position.
...
**B's snake mode is the only way to see the snake**, since it ships disabled inside `<template data-disabled="snake">`. The first switch into it unwraps the template and calls `vport._bootSnake()`, which `js/site.js` exposes for exactly this: on the live page boot finds no canvas, returns `false`, and nothing else in the snake runs. With the badge hidden the snake draws its own head segment (`badgeVisible` in `draw()` is `badge.offsetParent !== null`), so it doesn't read as a tail with nothing leading it — and `HEAD` falls back to the badge's *computed* width, because `offsetWidth` is 0 on a `display:none` element and a 0 head erases the whole taper.
