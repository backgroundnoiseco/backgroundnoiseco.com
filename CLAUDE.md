...
2. `.p-rail` — fixed vertical side rail (`00 Title / 01 Shadowbox / 02 Unicorn Porcupine / 03 Contact`; Prosession hidden). A `<nav aria-label="Sections">` of real `<button>`s (keyboard reachable; the driver mirrors `.on` into `aria-current`). Rotated via `writing-mode: vertical-rl` + `rotate(180deg)`. Left offset is `calc(var(--gutter) - 18px)` so it tracks the content column; below the cap this resolves to the original `left: 14px`.
   ...
   Every button but the last is `flex: 1` and carries an `::after` rule that eats its leftover space, so the labels are **joined by lines** rather than separated by gaps; the last is content-sized so it lands hard on the bottom edge. The rail is rotated 180°, which flips each button's internal order too — hence `flex-direction: row-reverse` on the button, without which the rule renders *above* its label.

   Type (`--rail-fs` / `--rail-ls`) takes the `min()` of a width- and an `svh`-based term, and the connector margins are in `em`, so everything shrinks together on short windows. That budget is tight: at 1100×430 the labels plus margins need ~293px against a ~295px span, which is why the floors are 6px/1.3px and the margins `.9em`. **Raising any of those, or lengthening a label, can clip the rail on short windows** — check 1100×430 after such a change.
