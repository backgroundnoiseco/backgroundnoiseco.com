...
Hold **shift** for 10× finer movement, **R** to reset to the stylesheet's own values, **H** to hide the panel (it collapses to a `tune ▸` chip rather than vanishing, so there's always a way back), **G** to hide the handles for a clean look at the composition, **B** to hide the badge. All three toggle independently, so you can keep the numbers up while the grips are out of the way; dragging the wordmark and badge still works with handles hidden. Hiding is a view toggle only — it never changes the reported values, and **R** un-hides the badge before reseeding (its `offsetWidth` reads 0 while hidden, which would otherwise poison the reset).
