...
- **Badge `top` at 1000px must be `-(badge width at 1000px)`**, so the badge's bottom edge lands on the wordmark's top edge. Change the width pair and `top` has to follow, or the badge sinks into the first line.
- **Hero `padding-bottom` is derived as `padTop - --hero-v-offset`, never given its own pair.** The block is centred and far shorter than the frame, so vertical padding clears nothing — its only effect is that `align-items:center` puts the block's centre at `(frameH + padTop - padBottom)/2`. The *difference* is the entire vertical position; two independent ramps converge and walk the wordmark upward as the window narrows. Don't raise `--hero-v-offset` past 40px either (padTop's small keyframe), or padBottom goes negative, clamps to 0, and the difference starts varying again. To move the hero vertically use `--hero-drop`, a margin on `.hero-block` with no floor to hit — it shifts by *half* its value, since the margin box is what gets centred.
