...
- The badge is a **link to Unicorn Porcupine on the App Store**, deliberately `aria-hidden` + `tabindex="-1"`. It lives inside the `<h1>`, so an exposed link would graft its label onto the heading's accessible name; the same URL is already keyboard reachable as the Porcupine panel's CTA, so nothing is lost by making this one a pointer-only shortcut. Where a glyph overlaps the badge the h1's inline content wins the hit (badge is `z-index: -1`, below it) — the rest of it is clickable.
- The badge's `z-index: -1` is what puts it **behind the wordmark's glyphs** like the rest of the snake. A negative-z positioned descendant paints below its ancestor's inline content but above the ancestor's background, and `.hero-block` (z-index 2) as a whole still sits above the snake canvas (z-index 1) — so the paint order ends up body, badge, wordmark.
...
- The outer rings' tinted white (`WHITE` in `js/site.js`) and the gutter's `--ringOverlay` are the ring pink lifted toward white, so the ramp stays one family — **re-derive both if the pink changes**, or the outer rings drift magenta while the centre reads red. On Porcupine, opacity reaches **1** (so no dark bg bleeds through and the outer ring reads at full colour) and each ring gets an interpolated `--c` ramping **warm-white outermost → pink innermost**. Colour is keyed to **index over count** — every ring is one even step toward the centre colour, so the ramp spans warm-white → full pink at any ring count. It was previously keyed to pixel depth against a fixed 240px reference so a given ring kept its colour on resize; that left mobile (5 rings instead of 8) stopping at `rgb(238,122,180)` and the project title washing out against it. **Trade-off deliberately re-accepted: rings now recolour as the window, and thus the ring count, changes.** To recolour another panel, give it its own `--rect-mix` expression and per-ring `--c` endpoints — the machinery generalises.
