...
- The badge is **anchored to the second concentric rectangle** — `top: 40px`, `right: calc(var(--cap) + 40px)` — so its top and right edges land exactly on ring 2's, verified at 0.0px across 360→1920. That is only expressible because its containing block is `.panel--hero` rather than the `<h1>`; the h1 is vertically centred and moves with the window. Only `width` is a keyframe pair now, so resizing the badge no longer drags its position.
