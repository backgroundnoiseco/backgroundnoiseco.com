...
- **The badge is mobile-only** — hidden above 565px, the same boundary the info bar's feature list switches at. It is absolutely positioned, so hiding it shifts nothing. Anchoring it never settled at desktop widths: it hangs off the end of the wordmark (the h1 shrink-wraps, badge at `left:100%`), so it inherits the h1's width *and* slides left as a share of the window past the 1000px cap, where everything freezes its offset from the 936px band. Only 360→565 of its keyframe pairs is ever rendered; the 1000px endpoints are notional. Two live couplings if it ever returns at desktop: it is `aspect-ratio: 1`, so widening it moves its **bottom** edge down by the same amount, and changing `--hero-size` moves it sideways.
